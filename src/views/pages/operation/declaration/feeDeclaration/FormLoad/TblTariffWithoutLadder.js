...
const TblTariffWithoutLadder = (props) => {
  ...
  const handleSave = () => {
    try {
      feeDeclaration.updateDetailBTO_1(editingId, editingData).then(
        (response) => {
          showAlertSuccess('Cập nhâp thành công');
          setEditingId(null);
          setEditingData({});
          props.handleLoadBTO1(props.id);
        },
        (error) => {
  ...
  const handleAdd = () => {
    try {
      ...
      newItem.channel = '-1';
      feeDeclaration.addNewDetailBTO_1(newItem).then(
        (response) => {
  ...
  const handleDeleteBTO01 = (confirmed) => {
    if (confirmed) {
      feeDeclaration.deleteBTO_01(idSelected).then(
        (response) => {
  ...
  const headerBTO1 = ['Số tiền gd từ', 'Số tiền gd đến', 'Phí CS TCPL', 'Phí CS TCNL', 'Phí DV TCPL', 'Phí DV TCNL', 'Hành Động'];
  const handleDetail = (value) => {
    setnumberSeleted(value);
  };
              ...
              {editingId === object.tariffId ? (
                ...
                <>
                  <IconButton title="Chọn">
                    <DetailIcon color="primary" className={classes.icons} onClick={() => handleDetail(xValue)} />
                  </IconButton>
                  <IconButton title="Sửa" onClick={() => handleEdit(object.tariffId, object)}>
            ...
            <Grid container spacing={gridSpacing}>
              <Grid item xs={12} md={10.7}>
                <Typography variant="h4" gutterBottom>
                  Mức phí chi tiết không bậc thang: {props.name}
                </Typography>
              </Grid>
...
export default TblTariffWithoutLadder;
