...
import ParticipantConfigSe from 'services/AssignParticipant/AssignParticipantByParticipant.service';
...
const FeeParticipantConfig = () => {
  ...
  const [businessSvcCode, setbusinessSvcCode] = useState(null);
  ...
  const getBusinessSvcCode = () => {
    CommonService.getInfChannelId()
      .then(
        (response) => {
          convertBusinessSvcCode(response.data);
        },
        (error) => {
          setMessage((error.response && error.response.data && error.response.data.message) || error.message || error.toString());
          showAlert(message);
  ...
  const listAllElements = (object) => {
    handleLoadingClick();
    ParticipantConfigSe.searchAssignParti(paging, object)
      .then(
        (response) => {
          handleLoadingClick();
  ...
  return (
    <MainCard title="Khai báo biểu phí cho tổ chức thành viên">
      <SearchForm handleSearch1={handleSearch} typeFee={typeFee} typebzSrc={typebzSrc} />
      <br />
      <br />
      <ParticapantTable
        data={pageInfo}
        ...
        businessSvcCode={businessSvcCode}
      />
