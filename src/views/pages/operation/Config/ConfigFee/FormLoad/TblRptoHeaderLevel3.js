...
const TblRptoHeaderLevel3 = (props) => {
  ...
  const handleSave = () => {
    try {
      configFee.updateConfigFee03(editingId, editingData).then(
        (response) => {
  ...
  const handleAdd = () => {
    try {
      newItem.level2Id = props.id;
      configFee.addConfigFee03(newItem).then(
        (response) => {
  ...
  const handleDeleteBTO01 = (confirmed) => {
    if (confirmed) {
      configFee.deleteConfigFee03(idSelected).then(
        (response) => {
    ...
    if (addClick) {
      var tableAdd = (
        <TableRow>
          ...
          <TableCell className="align-middle text-center no-wrap-box">
            <TextField value={newItem.headerOrder} type="number" name="headerOrder" onChange={handleChangeAdd} />
          </TableCell>
...
export default TblRptoHeaderLevel3;
