...
const TblRptoHeaderLevel2 = (props) => {
  ...
  const handleSave = () => {
    try {
      configFee.updateConfigFee02(editingId, editingData).then(
        (response) => {
  ...
  const handleAdd = () => {
    try {
      newItem.level1Id = props.id;
      configFee.addConfigFee02(newItem).then(
        (response) => {
  ...
  const handleDeleteBTO01 = (confirmed) => {
    if (confirmed) {
      configFee.deleteConfigFee02(idSelected).then(
        (response) => {
...
export default TblRptoHeaderLevel2;
