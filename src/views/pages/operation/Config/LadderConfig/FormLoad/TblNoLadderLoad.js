...
const TblNoLadderLoad = (props) => {
  ...
  const handleSave = () => {
    try {
      ...
      ServiceCallAPI.updateNoLadder(editingId, editingData).then(
        (response) => {
  ...
  const handleAdd = () => {
    try {
      ...
      ServiceCallAPI.addNoLadder(newItem).then(
        (response) => {
  ...
  const handleDeleteBTO01 = (confirmed) => {
    if (confirmed) {
      ServiceCallAPI.deleteNoLadder(idSelected).then(
        (response) => {
            ...
            <Grid container spacing={gridSpacing}>
              <Grid item xs={12} md={10.7}>
                <Typography variant="h4" gutterBottom>
                  Cấu hình khoảng tiền không bậc thang
                </Typography>
              </Grid>
