...
const TblNoLadderLoad = (props) => {
  ...
  const headerBTO1 = ['STT', 'Số tiền từ', 'Sô tiền đến', 'Hành động'];
  ...
  const handleSave = () => {
    try {
      if (!editingData['valueRangeMin'] || editingData['valueRangeMin'] < 0) {
        showAlert('"Số tiền từ" có giá trị không phù hợp (Vd: lơn hơn 0)');
        return;
      }
      if (!editingData['valueRangeMax'] || editingData['valueRangeMax'] > 499999999) {
        showAlert('"Sô tiền đến" có giá trị không phù hợp (Vd: nhỏ hơn 499999999)');
        return;
      }
      editingData['id'] = props.id;
      ServiceCallAPI.updateLadder02(editingId, editingData).then(
        (response) => {
  ...
  const handleAdd = () => {
    try {
      if (!newItem['valueRangeMin'] || newItem['valueRangeMin'] < 0) {
        showAlert('"Số tiền từ" có giá trị không phù hợp');
        return;
      }
      if (!newItem['valueRangeMax'] || newItem['valueRangeMax'] > 499999999) {
        showAlert('"Sô tiền đến" có giá trị không phù hợp (Vd: nhỏ hơn 499999999)');
        return;
      }
      newItem['ladderId'] = props.id;
      ServiceCallAPI.addNewLadder02(newItem).then(
        (response) => {
  ...
  const handleDeleteBTO01 = (confirmed) => {
    if (confirmed) {
      ServiceCallAPI.deleteLadder02(idSelected).then(
        (response) => {
      ...
      listElements.forEach((object) => {
        ...
        var tableRow = (
          <TableRow key={rowIndex}>
            ...
            <TableCell className={'align-middle text-center no-wrap-box '} style={{ width: '30%' }}>
              {editingId === object.id ? (
            ...
            <TableCell className={'align-middle text-center no-wrap-box '} style={{ width: '30%' }}>
              {editingId === object.id ? (
                <TextField value={editingData.valueRangeMax} type="number" name="valueRangeMax" onChange={handleChange} />
              ) : (
            ...
            <Grid container spacing={gridSpacing}>
              <Grid item xs={12} md={10.7}>
                <Typography variant="h4" gutterBottom>
                  Cấu hình khoảng tiền trong bậc thang
                </Typography>
              </Grid>
...
export default TblNoLadderLoad;
