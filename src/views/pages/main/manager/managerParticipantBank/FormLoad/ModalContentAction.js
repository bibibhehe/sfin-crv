...
import ServiceAPI from 'services/ManagerMerchant/ParticipantBank.service';
...
const ModalContentAction = (props) => {
  ...
  const formatShow = [
    { name: 'bankId', label: 'Bank ID', type: 'text', object: null, required: true, show: true },
    { name: 'bic', label: 'BIC', type: 'text', object: null, required: true, show: true },
    { name: 'bankShortName', label: '	Tên viết tắt', type: 'text', object: null, required: true, show: true },
    { name: 'bankFullNameVi', label: 'Tên đầy đủ', type: 'text', object: null, required: true, show: true },
    { name: 'bankFullNameEn', label: 'Tên đầy đủ ENG', type: 'text', object: null, required: true, show: true },
  ...
  const handleSubmit = () => {
    ...
    if (filtersInput?.bankReceiveCode?.length > 2) {
      props.showError('Trường Bank Code có độ dài nhở hơn hoặc bằng 2');
      return;
    }
      ...
      <Dialog onClose={handleClose} open={props.open} maxWidth="xl" keepMounted fullWidth={false}>
        <DialogTitle style={{ fontSize: '1.3rem', color: 'white', fontWeight: 'bold', backgroundColor: '#2196f3' }}>
          <Grid container spacing={2}>
            <Grid item xs={11}>
              {props.actionModal.type == 'add' ? 'Thêm mới tổ chức thành viên' : 'Sửa thông tin tổ chức thành viên'}
            </Grid>
            <Grid item xs={1}>
              <Button
                onClick={handleClose}
