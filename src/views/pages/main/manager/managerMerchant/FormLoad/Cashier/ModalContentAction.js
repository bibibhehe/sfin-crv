...
const ModalContentAction = (props) => {
  ...
  const handleAddMerchant = (object) => {
    ServiceAPI.addNewCashier(object, props.id).then(
      (response) => {
  ...
  const handleUpdateMerchant = (object) => {
    ServiceAPI.updateCashier(object).then(
      (response) => {
  ...
  const formatShow = [
    { name: 'cashierCode', label: 'Cashier Code', type: 'text', object: null, required: true, show: true },
    { name: 'cashierName', label: 'Tên Cashier', type: 'text', object: null, required: true, show: true },
  ];
      ...
      <Dialog onClose={handleClose} open={props.open} maxWidth="xl" keepMounted fullWidth={false}>
        <DialogTitle style={{ fontSize: '1.3rem', color: 'white', fontWeight: 'bold', backgroundColor: '#2196f3' }}>
          <Grid container spacing={2}>
            <Grid item xs={11}>
              {props.actionModal.type == 'add' ? 'Thêm Cashier Mới' : 'Sửa Thông Tin Cashier'}
            </Grid>
            <Grid item xs={1}>
              <Button
                onClick={handleClose}
